# run_ps_helper.ps1
# 開発環境用スクリプトを実行するためのヘルパーPowerShellスクリプト
...
while ($i -lt $args.Count) {
    if ($args[$i] -eq "-m" -and $i+1 -lt $args.Count) {
        ...
        # ヘルプメッセージの表示
        Write-Host "使用方法:"
        Write-Host "  .\run_dev_launcher.bat [オプション]"
        Write-Host ""
        Write-Host "オプション:"
        ...
        Write-Host ""
        Write-Host "例:"
        Write-Host "  .\run_dev_launcher.bat"
        Write-Host "  .\run_dev_launcher.bat -m utils.spreadsheet"
        exit 0
    } else {
        $i++
    }
