...
## WebDriverManagerのバージョン互換性確保

特定のバージョンのChromeブラウザに対応するドライバーを明示的に指定する機能が追加されました。これにより、以下のメリットが得られます：

### 機能概要

- 特定のバージョンのChromeに対応するドライバーを指定可能
- 古いブラウザバージョンでのテスト実行をサポート
- 環境設定でバージョン指定することでチーム全体で一貫性を確保

### 設定方法

`config/settings.ini` ファイルの `[BROWSER]` セクションで設定できます：

```ini
[BROWSER]
# 他の設定...
chrome_version = 88.0.4324.96  # 特定のバージョンを指定
```

コメントアウトするか削除すると、現在のブラウザに適したドライバーが自動検出されます。

### 使用例

```python
# 設定ファイルから自動的にバージョンを読み込む
browser = Browser(logger=logger, headless=True)
browser.setup()  # 設定ファイルのchrome_versionが使用される

# または明示的にバージョンを指定
browser = Browser(logger=logger, headless=True)
browser.setup(browser_version="88.0.4324.96")
```

### 利点

1. 特定のブラウザバージョンに固定されている環境でのテスト安定性向上
2. CI/CD環境での一貫性確保
3. 古いバージョンへのフォールバックが必要な場合の動作保証

### 注意点

特定のバージョンを指定する場合は、そのバージョンに対応するドライバーがWebDriverManagerで入手可能であることを確認してください。
